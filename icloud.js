...
module.exports = function() {
    ...
    */
    ...
    function findme(cb) {
        ...
        var url = session.webservices.findme.url.replace(':443', '');

        req.post({
            url : session.webservices.findme.url + "/fmipservice/client/web/initClient",
            headers : {
                host : session.webservices.findme.url.split('//')[1].split(':')[0],
            }
        }, function(err, resp, body) {
            if (err) return cb(err);
